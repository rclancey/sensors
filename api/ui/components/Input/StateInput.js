...
import { countries } from './CountryInput.js';
...
export const StateMenu = ({ country, onChange, ...props }) => {
  ...
  if (groups.length === 0) {
    return html`<${StringInput} onChange=${onChange} ${...props} />`;
  }
  if (groups.length === 1) {
    return html`<${MenuInput} options=${groups[0].options} onChange=${callback} empty ${...props} />`;
  }
  return html`<${MenuInput} groups=${groups} onChange=${callback} empty ${...props} />`;
};
